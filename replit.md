...
- **RESOLVED: Page refresh "not found" errors** - Fixed client-side routing issues where refreshing pages would show "not found" messages. Removed conflicting Express static file middleware in development mode and let Vite handle all static assets and SPA routing properly. Added cache-busting headers for development while maintaining production fallback routing.
