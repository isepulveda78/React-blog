...
Sitemap: ${baseUrl}/sitemap.xml`;
  ...
  app.post('/api/audio-files', upload.single('audioFile'), async (req, res) => {
    try {
      if (!req.file) {
      ...
      const { name } = req.body;
      if (!name) {
        return res.status(400).json({ error: 'Audio file name is required' });
      }
      ...
      const filePath = path.join(__dirname, '../client/public/sounds', fileName);
      ...
      const audioFile = await storage.createAudioFile({
        ...
        uploadedByName: req.session.user?.name || req.session.user?.username || 'Anonymous'
      });
  ...
  app.delete('/api/audio-files/:id', async (req, res) => {
    try {
      ...
      const fs = require('fs');
      const path = require('path');
      const filePath = path.join(__dirname, '../client/public', audioFile.path);
